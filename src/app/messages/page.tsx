...
import { prefetchChatMessagesCache } from "@/lib/chat-message-cache";
...
function MessagesPageContent() {
    ...
    // Global qidiruv faqat @username bo‘yicha; telefon raqam ko‘rinmaydi
    const handleSearch = async (query: string) => {
        ...
        setIsSearching(true);
        try {
            const res = await apiFetch(`/api/users/search?q=${encodeURIComponent(query.trim())}&searchBy=username`);
            if (res.ok) {
                ...
                const mapped = list.map((u: any) => ({
                    ...
                    username: u.username,
                    message: 'Foydalanuvchi nomi',
                    avatar: u.avatar_url || u.avatar || 'use_initials',
                    type: 'contact',
                    isGlobal: true,
                    // Telefon raqamni frontendga yuborilmaydi (maxfiylik)
                }));
