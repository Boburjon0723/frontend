...
function MentorProfileHeader({
    ...
    const expertRoleLabel = getStudentPanelExpertLabel(panelMode);
...
export default function StudentDashboard({ user, sessionId, sessionStyle = 'mentor', onLeave }: StudentDashboardProps) {
    ...
    const expertRoleLabel = getStudentPanelExpertLabel(style);
    const selfRoleLabel = getStudentSelfRoleLabel(style);
    ...
    /** Konsultatsiya (mijoz): chat o‘rniga doska paneli */
    const [showWhiteboardPanel, setShowWhiteboardPanel] = useState(() => !isClassroomMentor);
    ...
    const complianceBlock = getExpertComplianceNotice(style, 'client');
    ...
    useEffect(() => {
        ...
        const onChatLike = (msg: any) => {
            ...
            if (isClassroomMentor && /📝|Viktorina|\*\*Viktorina/i.test(text)) {
                setShowQuizzes(true);
                setStudentBanner('Chatda viktorina xabari — «Test» paneli ochildi.');
                setQuizPanelHighlight(true);
            } else if (isClassroomMentor && /⚡|Tezkor so['ʼ]rov/i.test(text)) {
                setStudentBanner('Chatda tezkor so‘rov haqida xabar.');
                setQuizPanelHighlight(true);
            } else if (/📎|Material yuklandi/i.test(text)) {
                setStudentBanner('Chatda yangi material haqida xabar — «Fayllar» ni tekshiring.');
            }
    ...
    useEffect(() => {
        ...
        const handleQuizStart = (data: { sessionId?: string, quizId?: string, quizDetails: { id?: number | string; title: string, isQuickPoll?: boolean, questions?: any[] } }) => {
            ...
            if (details?.isQuickPoll) {
                ...
                setStudentBanner('Ustoz tezkor so‘rov yubordi — javob bering.');
                setQuizPanelHighlight(true);
            } else {
                const baseTitle = details?.title ?? 'Viktorina';
        ...
        const handleStudentKicked = (data: { sessionId: string }) => {
            if (data.sessionId === sessionId) {
                alert(
                    isClassroomMentor
                        ? "Siz mentor tomonidan darsdan chiqarildingiz."
                        : `Siz ${expertRoleLabel.toLowerCase()} tomonidan sessiyadan chiqarildingiz.`
                );
                onLeave();
            }
        };
        ...
        const handleLessonEnded = (data: { sessionId?: string; message?: string }) => {
            ...
            if (isClassroomMentor) {
                alert(
                    data?.message ||
                        "Dars yakunlandi. Ustoz darsni tugatdi."
                );
                onLeave();
                return;
            }
            ...
            setStudentBanner('Xizmat yakunlandi.');
    ...
    }, [socket, sessionId, onLeave, user?.id, isClassroomMentor, expertRoleLabel]);
    ...
    const handleSubmitQuiz = (quizId: string | number) => {
        ...
        if (!answers) return alert("Avval barcha savollarga javob bering.");
    ...
    useEffect(() => {
        const fetchToken = async () => {
            if (!sessionId || sessionId === 'demo-session-id') {
                setError(
                    isClassroomMentor
                        ? "Sessiya topilmadi yoki tugagan. Mentor yuborgan havolani qayta tekshiring."
                        : `Sessiya topilmadi yoki tugagan. ${expertRoleLabel} yuborgan havolani yoki chatdagi tugmani qayta tekshiring.`
                );
                setLoading(false);
                return;
            }
                        ...
                        <div className="hidden lg:flex items-center gap-2 rounded-2xl border border-white/10 bg-black/45 px-2 py-1.5 backdrop-blur-xl">
                            ...
                            {isClassroomMentor ? (
                                <button
                                    ...
                                    title={handRaised ? "Qo'lni tushirish" : 'Savolim bor'}
                                >
                                    <Hand className="h-4 w-4" />
                                    <span className="hidden xl:inline">{handRaised ? 'Tushirish' : 'Qo‘l'}</span>
                                </button>
                            ) : null}
                            <ControlToggleButton
                                                    ...
                                                    <div key={String(quiz.id)} className="mb-8 last:mb-0 animate-in fade-in zoom-in-95 duration-500">
                                                        <div className="mb-5 rounded-2xl border border-white/10 bg-gradient-to-br from-fuchsia-500/10 to-sky-500/10 p-4 sm:p-5">
                                                            ...
                                                            <p className="mt-1 text-xs font-medium text-white/50">
                                                                {quiz.questions?.length ?? 0} ta savol
                                                            </p>
                                                        </div>
