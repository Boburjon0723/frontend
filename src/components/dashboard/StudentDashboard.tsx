    ...
    useEffect(() => {
        ...
        const handleQuizStart = (data: { sessionId?: string, quizId?: string, quizDetails: { id?: number | string; title: string, isQuickPoll?: boolean, questions?: any[] } }) => {
            ...
            if (details?.isQuickPoll) {
                ...
                const quiz = { id: details?.id ?? data.quizId ?? Date.now(), ...details, title: baseTitle };
                ...
                <div className="absolute bottom-10 inset-x-0 flex justify-center z-40 pointer-events-none">
                    <div className="flex items-center gap-3 p-3 bg-black/60 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] shadow-2xl pointer-events-auto scale-110">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl font-bold text-[10px] tracking-widest transition-all ${handRaised ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/30' : 'bg-white/5 hover:bg-amber-500/20 text-white/80 hover:text-amber-300 border border-white/10'}`}
                            title={handRaised ? "Qo'lni tushirish" : "Savolim bor"}
                        >
                            <Hand className="w-5 h-5" />
                            {handRaised ? "Qo'l tushirish" : "Savolim bor"}
                        </button>
                        <ControlToggleButton
