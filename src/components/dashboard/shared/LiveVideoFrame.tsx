...
interface LiveVideoFrameProps {
    ...
    immersive?: boolean;
    /** Qo'l ko'targan talabalar — mentor panelda ularning videolari shu qatorda navbat bilan chiqadi */
...
export function LiveVideoFrame({
    ...
    const raisedIds = Object.keys(handsRaised);
                        ...
                        {isMentor && raisedTracksInOrder.length > 0 && (
                            <div className="shrink-0 space-y-2">
                                ...
                                {raisedTracksInOrder.map((track, i) => (
                                    <div key={track.participant.identity} className="w-full aspect-video rounded-xl overflow-hidden relative bg-slate-800 ring-2 ring-amber-500/50 shrink-0">
                                        <ParticipantTile trackRef={track} className="w-full h-full [&>video]:object-cover" />
                                        <div className="absolute bottom-1 left-1 text-[9px] font-bold text-white bg-black/70 px-1.5 py-0.5 rounded truncate max-w-full">
                                            {handsRaised[track.participant.identity] || `Talaba ${i + 1}`}
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    {isMentor && raisedTracksInOrder.length > 0 && (
                        <div className="shrink-0 h-[140px] flex flex-col bg-amber-500/10 border-b border-amber-500/20">
                            <div className="px-3 py-1.5 flex items-center gap-2 border-b border-amber-500/20">
                                ...
                                <span className="text-xs font-bold text-amber-200 uppercase tracking-widest">Savol bermoqda — videolar</span>
                            </div>
                            <div className="flex-1 flex gap-2 p-2 overflow-x-auto overflow-y-hidden custom-scrollbar">
                                {raisedTracksInOrder.map((track, i) => (
                                    <div key={track.participant.identity} className="h-full min-w-[180px] max-w-[220px] aspect-video rounded-xl overflow-hidden relative bg-slate-800 shrink-0 ring-2 ring-amber-500/50 shadow-lg">
                                        <ParticipantTile trackRef={track} className="w-full h-full [&>video]:object-cover" />
                                        <div className="absolute bottom-1.5 left-1.5 right-1.5 text-[10px] font-bold text-white bg-black/70 px-2 py-1 rounded-lg backdrop-blur-md truncate border border-white/10">
                                            {handsRaised[track.participant.identity] || track.participant.identity || `Talaba ${i + 1}`}
                                        </div>
                                    </div>
                                ))}
                            </div>
