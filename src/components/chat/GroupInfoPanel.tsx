...
export default function GroupInfoPanel({ chat, onClose, onDeleted, onLeft, onGroupUpdated, onChatNotFound }: GroupInfoPanelProps) {
    ...
    const isCreator = fullChatDetails?.creator_id && currentUser?.id && fullChatDetails.creator_id === currentUser.id;

    const handleDeleteGroup = async () => {
        if (!chat || !confirm("Guruh butunlay o‘chiriladi. Rostan ham davom etasizmi?")) return;
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert(data?.message || "Guruhni o‘chirib bo‘lmadi.");
            }
        } catch (err) {
    ...
    const handleLeaveGroup = async () => {
        if (!chat || !confirm("Guruhdan chiqasizmi?")) return;
        setActionLoading('leave');
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(`${API_URL}/api/chats/${chat.id || chat._id}/leave`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = res.ok ? null : await res.json().catch(() => ({}));
            if (res.ok) {
                ...
                alert(data?.message || "Guruhdan chiqib bo‘lmadi.");
            }
        } catch (err) {
    ...
    const handleUpdateName = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert(data?.message || "Nom yangilanmadi.");
            }
        } catch (err) {
    ...
    const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!chat || !file || !file.type.startsWith('image/')) return;
        e.target.value = '';
        ...
        try {
            ...
            if (!chatId) {
                alert('Guruh ID topilmadi. Guruh ma\'lumotlarini yuklashni kuting.');
                return;
            }
            const patchUrl = `${API_URL}/api/chats/${chatId}`;
            console.log('[GroupInfoPanel] PATCH guruh rasm:', patchUrl, 'chatId:', chatId);
            ...
            if (res.ok) {
                ...
                if (res.status === 404) {
                    onChatNotFound?.();
                    alert('Guruh backendda topilmadi. Ro\'yxat yangilandi – faqat mavjud guruhlarda profil rasmini yuklashingiz mumkin.');
                } else {
                    alert(errData?.message || 'Rasm yangilanmadi.');
                }
            }
        } catch (err: any) {
            console.error('Update group avatar:', err);
            const msg = err?.message || (err?.response?.data?.message) || 'Rasm yuklanmadi. .env.local da NEXT_PUBLIC_API_URL ni Railway manziliga qiling.';
            alert(msg);
        } finally {
            ...
            <div className="px-3 flex-1">
                ...
                <div className="space-y-2 pb-4">
                    {participants.map((member: any) => {
                        ...
                        const isCreatorMember = fullChatDetails?.creator_id && member.id === fullChatDetails.creator_id;
                        return (
